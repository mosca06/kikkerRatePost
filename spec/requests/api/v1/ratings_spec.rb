  ...
  describe 'POST /api/v1/ratings' do
    ...
    context 'quando o usuário já avaliou o post' do
      ...
      it 'não permite avaliar novamente' do
        ...
        expect(response.parsed_body['errors']).to include('User already rated this Post.')
      end
    end
    ...
    context 'com valor fora do permitido' do
      it 'retorna erro de validação' do
        ...
        expect(response).to have_http_status(:unprocessable_entity)
        expect(response.parsed_body['errors']).to include('Value must be between 1 and 5.')
      end
    end
    ...
    context 'com dados incompletos' do
      it 'retorna erro de validação' do
        ...
        expect(response).to have_http_status(:unprocessable_entity)
        expect(response.parsed_body['errors']).to include('User must exist')
      end
    end
  end
end
